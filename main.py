...
# Optional imports dengan proper error handling
try:
...
# Parse OWNER_IDS dengan benar
_owner_ids_raw = os.getenv("OWNER_IDS", "")
OWNER_IDS = [int(x) for x in _owner_ids_raw.split(",") if x.strip().isdigit() and int(x) > 0]
if not OWNER_IDS:
    logger.warning("OWNER_IDS not set or invalid!")
...
def _fetch_panel_config_sync(force=False):
    """Synchronous version for panel config fetch"""
    global _panel_config
    if not CONFIG_PANEL_URL or not CONFIG_BOT_SECRET:
        return None
    
    with _sync_panel_lock:
        now = time.time()
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.info(f"Panel synced: {len(_panel_config['keys'])} keys")
                return _panel_config
        except Exception as e:
            logger.warning(f"Panel fetch: {e}")
    
    return None


def fetch_panel_config(force=False):
    """Wrapper untuk fetch panel config"""
    return _fetch_panel_config_sync(force)
...
DEFAULT_MODELS = {
    ...
    "tavily": {"e": "🔍", "n": "Tavily", "d": "Web Search", "c": "main", "p": "tavily", "m": "search"},
    ...
    "pf_openai": {"e": "🆓", "n": "PollFree-OpenAI", "d": "OpenAI Free", "c": "pollinations_free", "p": "pollinations_free", "m": "openai"},
    "poll_free": {"e": "🌸", "n": "PollFree-Auto", "d": "Auto Free", "c": "pollinations_free", "p": "pollinations_free", "m": "auto"},
    "pa_openai": {"e": "🔑", "n": "PollAPI-OpenAI", "d": "OpenAI API", "c": "pollinations_api", "p": "pollinations_api", "m": "openai"}
}

IMG_MODELS = {
    "flux": ("🎨", "Flux", "Standard"),
    "flux_pro": ("⚡", "Flux Pro", "Pro"),
    "turbo": ("🚀", "Turbo", "Fast"),
...
def get_models():
    ...
    if config and config.get("models"):
        return {**DEFAULT_MODELS, **config["models"]}
...
def get_user_model(uid):
    config = fetch_panel_config()
    if config and str(uid) in config.get("user_models", {}):
        return config["user_models"][str(uid)]
    return None


def get_default_model():
    return get_panel_setting("default_model", "groq")
...
class ShieldAPI:
    ...
    def _req(self, method, ep, data=None):
        ...
        try:
            url = f"{self.url}{ep}"
            if method == "GET":
                r = get_requests().get(url, headers=self._h(), timeout=self.timeout)
            elif method == "POST":
                r = get_requests().post(url, headers=self._h(), json=data or {}, timeout=self.timeout)
            elif method == "DELETE":
                r = get_requests().delete(url, headers=self._h(), json=data, timeout=self.timeout)
            else:
    ...
    def sessions(self):
        return self._req("GET", "/api/admin/sessions")
    
    def logs(self):
        return self._req("GET", "/api/admin/logs")
    
    def bans(self):
        return self._req("GET", "/api/admin/bans")
    
    def whitelist(self):
        return self._req("GET", "/api/admin/whitelist")
    
    def suspended(self):
        return self._req("GET", "/api/admin/suspended")
    ...
    def unsuspend(self, sus_type, value):
        return self._req("POST", "/api/admin/unsuspend", {
            "type": sus_type,
            "value": value
        })
    
    def kill(self, sid, reason="Via Discord"):
        return self._req("POST", "/api/admin/kill-session", {
            "sessionId": sid,
            "reason": reason
        })
    
    def clear_sessions(self):
        return self._req("POST", "/api/admin/sessions/clear")
    
    def clear_logs(self):
        return self._req("POST", "/api/admin/logs/clear")
    
    def clear_cache(self):
        return self._req("POST", "/api/admin/cache/clear")
...
class Database:
    ...
    # FIXED: Added 'users' field to stats
    def get_stats(self):
        with self.lock:
...
class Memory:
    ...
    def add(self, uid, role, content):
        ...
        with self.lock:
            now = time.time()
            self.conv[uid] = [m for m in self.conv[uid] if now - m.ts < timeout * 60]
            self.conv[uid].append(ChatMsg(role, content[:2500], now))
            if len(self.conv[uid]) > max_m:
                self.conv[uid] = self.conv[uid][-max_m:]
    
    def get(self, uid):
        with self.lock:
            now = time.time()
            self.conv[uid] = [m for m in self.conv[uid] if now - m.ts < 1800]
    ...
    def dump(self, url, use_cache=True):
        ...
        for name, func in methods:
            try:
                ...
                logger.debug(f"Dump method {name} failed: {e}")
...
def call_groq(msgs):
    ...
    try:
        ...
        if r and r.choices and len(r.choices) > 0:
...
def call_cerebras(msgs):
    ...
    try:
        ...
        r = get_requests().post(
            ...
            json={"model": m, "messages": msgs, "max_tokens": 4096},
            timeout=30
        )
        if r.status_code == 200:
            data = r.json()
...
def call_cloudflare(msgs):
    ...
    try:
        ...
        r = get_requests().post(
            f"https://api.cloudflare.com/client/v4/accounts/{acc}/ai/run/{m}",
        ...
        if r.status_code == 200:
            ...
            if d.get("success") and d.get("result"):
                return d["result"].get("response", "").strip()
        return None
    except Exception as e:
        logger.error(f"CF: {e}")
        return None
...
# FIXED: Async-compatible replicate call
def _call_replicate_sync(msgs):
    """Synchronous replicate call to be run in executor"""
    ...
    try:
        ...
        r = get_requests().post(
            f"https://api.replicate.com/v1/models/{m}/predictions",
            headers={"Authorization": f"Bearer {k}", "Content-Type": "application/json"},
...
def call_replicate(msgs):
    """Wrapper that can be used synchronously"""
    return _call_replicate_sync(msgs)


async def call_replicate_async(msgs):
    """Async version of replicate call"""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(executor, _call_replicate_sync, msgs)
...
def call_tavily(msgs):
    ...
    try:
        ...
        r = get_requests().post(
            ...
            json={"api_key": k, "query": q, "search_depth": "advanced", "max_results": 5},
            timeout=20
        )
        if r.status_code == 200:
            d = r.json()
...
def call_openrouter(msgs, model_key):
    ...
    try:
        ...
        if r.status_code == 200:
            d = r.json()
            if d.get("choices") and len(d["choices"]) > 0:
...
def call_pollinations_api(msgs, model_key):
    ...
    if not k:
        return call_pollinations_free(msgs, model_key)
    try:
        mid = get_models().get(model_key, {}).get("m", "openai")
        r = get_requests().post(
            "https://text.pollinations.ai/",
...
def call_ai(model, msgs):
    ...
    if p == "groq":
        ...
        result = call_cloudflare(msgs)
    elif p == "cohere":
        result = call_cohere(msgs)
    elif p == "mistral":
        result = call_mistral(msgs)
    elif p == "together":
        result = call_together(msgs)
    elif p == "moonshot":
        result = call_moonshot(msgs)
    elif p == "huggingface":
        result = call_huggingface(msgs)
    elif p == "replicate":
        result = call_replicate(msgs)
    elif p == "openrouter":
        result = call_openrouter(msgs, model)
    elif p == "pollinations_free":
        result = call_pollinations_free(msgs, model)
    elif p == "pollinations_api":
        result = call_pollinations_api(msgs, model)
    elif p == "tavily":
        result = call_tavily(msgs)
...
FALLBACK = [
    ...
    ("poll_free", lambda m: call_pollinations_free(m, "poll_free"))
]
...
def ask_ai(prompt, uid=None):
    ...
    if not result:
        return "Maaf, semua AI sedang tidak tersedia.", "unknown"
...
# FIXED: Proper async image generation
async def gen_image(prompt, model="flux"):
    try:
        ...
        def fetch_image():
            return get_requests().get(url, timeout=120)
        
        r = await loop.run_in_executor(executor, fetch_image)
...
def split_msg(txt, lim=1900):
    ...
    while len(txt) > lim:
        ...
        if len(txt) >= lim and sp == 0:
            chunks.append(txt[:lim])
            txt = txt[lim:]
...
class ShieldInfoSelect(ui.Select):
    def __init__(self):
        opts = [
            ...
            discord.SelectOption(label="Suspended", value="sus", emoji="⏸️"),
            discord.SelectOption(label="Health", value="health", emoji="💚"),
            discord.SelectOption(label="Bot Stats", value="botstats", emoji="📈"),
        ...
        try:
            if a == "stats":
                ...
                if d.get("success") is False:
                    embed.description = f"❌ {d.get('error', 'Failed')}"
                elif isinstance(d, dict):
                    c = 0
                    for k, v in d.items():
                        if k not in ["success", "error", "message"] and c < 15:
                            embed.add_field(name=str(k).title(), value=f"`{str(v)[:50]}`", inline=True)
                            c += 1
            
            elif a == "sessions":
                d = shield.sessions()
                embed.title = "🔄 Sessions"
                if d.get("success") is False:
                    embed.description = f"❌ {d.get('error', 'Failed')}"
                else:
                    ...
                    if ss:
                        for idx, s in enumerate(ss[:8]):
                            embed.add_field(
                                name=f"#{idx + 1}",
                                value=f"ID: `{str(s.get('id', '?'))[:10]}`\nUser: `{str(s.get('userId', '?'))[:10]}`",
                                inline=True
                            )
                    else:
                ...
                if d.get("success") is False:
                    ...
                    if ll:
                        txt = []
                        for l in ll[:8]:
                            txt.append(f"`{l.get('time', '?')[:16]}` {l.get('service', '?')}")
                        embed.description = "\n".join(txt)[:4096]
                    else:
                        embed.description = "None"
                ...
                if d.get("success") is False:
                    ...
                    if bb:
                        for idx, b in enumerate(bb[:8]):
                            embed.add_field(name=f"#{b.get('id', '?')}", value=f"`{str(b.get('value', '?'))[:15]}`", inline=True)
                    else:
                        embed.description = "None"
            
            elif a == "wl":
                d = shield.whitelist()
                embed.title = "✅ Whitelist"
                if d.get("success") is False:
                    embed.description = f"❌ {d.get('error', 'Failed')}"
                else:
                    wl = d.get("whitelist", d if isinstance(d, list) else [])
                    if wl:
                        for idx, w in enumerate(wl[:8]):
                            embed.add_field(name=f"#{idx + 1}", value=f"`{str(w.get('value', w))[:20]}`", inline=True)
                    else:
                        embed.description = "None"
            
            elif a == "sus":
                d = shield.suspended()
                embed.title = "⏸️ Suspended"
                if d.get("success") is False:
                    embed.description = f"❌ {d.get('error', 'Failed')}"
                else:
                    sus = d.get("suspended", d if isinstance(d, list) else [])
                    if sus:
                        for idx, s in enumerate(sus[:8]):
                            embed.add_field(name=f"#{idx + 1}", value=f"`{str(s.get('value', s))[:20]}`", inline=True)
                    else:
                        embed.description = "None"
                ...
                if d.get("script"):
                    ...
                    embed.description = "❌ Not available"
...
class ShieldActionSelect(ui.Select):
    def __init__(self):
        opts = [
            discord.SelectOption(label="Clear Sessions", value="clear_s", emoji="🧹"),
            discord.SelectOption(label="Clear Logs", value="clear_l", emoji="🗑️"),
            discord.SelectOption(label="Clear Cache", value="clear_c", emoji="💾")
        ]
...
class ShieldManageSelect(ui.Select):
    ...
    async def callback(self, i: discord.Interaction):
        ...
        if a == "unban":
            ...
            l = "Session ID"
            p = "Enter Session ID"
        elif a in ["wl", "rem_wl", "sus", "unsus"]:
            t = "Manage User"
            l = "Type:Value"
            p = "userId:123 or hwid:ABC"
        else:
            t = "Ban User"
            l = "Type:Value"
            p = "userId:123 or hwid:ABC"
        
        class ShieldModal(ui.Modal, title=t):
            val = ui.TextInput(label=l, placeholder=p, required=True)
            reason = ui.TextInput(label="Reason", required=False, default="Via Discord")
            
            def __init__(modal_self, ac):
                super().__init__()
                modal_self.ac = ac
                if ac in ["unban", "unsus", "rem_wl", "kill"]:
                    modal_self.remove_item(modal_self.reason)
            
            async def on_submit(modal_self, mi: discord.Interaction):
                await mi.response.defer(ephemeral=True)
                v = modal_self.val.value.strip()
                r = modal_self.reason.value.strip() if hasattr(modal_self, "reason") and modal_self.reason.value else "Via Discord"
                res = {"success": False, "error": "Unknown"}
                
                try:
                    if modal_self.ac == "ban":
                        if ":" in v:
                            t_type, val = v.split(":", 1)
                        else:
                            t_type, val = "userId", v
                        res = shield.add_ban(t_type.strip(), val.strip(), r)
                    
                    elif modal_self.ac == "unban":
                        res = shield.rem_ban(v)
                    
                    elif modal_self.ac in ["wl", "rem_wl", "sus", "unsus"]:
                        if ":" in v:
                            t_type, val = v.split(":", 1)
                        else:
                            t_type, val = "userId", v
                        t_type, val = t_type.strip(), val.strip()
                        
                        if modal_self.ac == "wl":
                            res = shield.add_wl(t_type, val)
                        elif modal_self.ac == "rem_wl":
                            res = shield.rem_wl(t_type, val)
                        elif modal_self.ac == "sus":
                            res = shield.suspend(t_type, val, r)
                        elif modal_self.ac == "unsus":
                            res = shield.unsuspend(t_type, val)
                    
                    elif modal_self.ac == "kill":
                        res = shield.kill(v, r)
                    
                    if res.get("success") is not False and not res.get("error"):
                        await mi.followup.send(
                            f"✅ **Success!**\nAction: `{modal_self.ac}`\nValue: `{v}`\n-# *{WATERMARK}*",
                            ephemeral=True
                        )
                    else:
                        await mi.followup.send(
                    ...
                    await mi.followup.send(f"❌ **Error:** {str(e)[:100]}", ephemeral=True)
        
        await i.response.send_modal(ShieldModal(a))
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Activity(type=discord.ActivityType.listening, name=f"{PREFIX}help | {WATERMARK}")
    )
    ...
    if bot.user.mentioned_in(msg) and not msg.mention_everyone:
        ...
        if content:
            ...
            async with msg.channel.typing():
                # Run AI call in executor to prevent blocking
                loop = asyncio.get_event_loop()
...
async def cmd_lm(ctx):
    ...
    names = {
        ...
        "pollinations_free": "🆓 Poll Free",
        "pollinations_api": "🔑 Poll API",
        "custom": "⚙️ Custom"
    }
...
async def cmd_img(ctx, *, prompt: str = None):
    ...
    info = IMG_MODELS.get(model, ("🎨", "Flux", ""))
    ...
    try:
        ...
        if data:
            ...
            await st.edit(content=f"❌ Failed: {err}")
    except Exception as e:
        await st.edit(content=f"❌ Error: {str(e)[:50]}")
...
async def cmd_dump(ctx, url: str = None, *, flags: str = ""):
    ...
    if result["success"]:
        ...
        await st.edit(content=f"❌ {result.get('error', 'Failed')}")
...
async def cmd_shield(ctx):
    ...
    embed.add_field(
        name="URL",
        value=f"`{SHIELD_URL[:25]}...`" if len(SHIELD_URL) > 25 else f"`{SHIELD_URL or 'Not Set'}`",
        inline=True
    )
    embed.set_footer(text=WATERMARK)
...
async def cmd_sync(ctx):
    ...
    if config and config.get("last_fetch", 0) > 0:
        ...
        await st.edit(content="❌ Failed to sync")
...
async def cmd_status(ctx):
    ...
    config = fetch_panel_config()
    panel_ok = config and config.get("last_fetch", 0) > 0
    ...
    keys = [
        ...
        ("OpenRouter", "openrouter"), ("Mistral", "mistral"), ("Together", "together"),
    ...
    embed.add_field(
        name="⚙️ Config",
        value=f"Default: `{get_default_model()}`\nModels: `{len(get_models())}`\nServers: `{len(bot.guilds)}`",
        inline=True
    )
    embed.set_footer(text=WATERMARK)
...
async def cmd_testai(ctx):
    ...
    providers = [
        ...
        ("PollFree", lambda: call_pollinations_free(test, "poll_free"), True)
    ]
...
async def cmd_au(ctx, user: discord.User = None, *, models: str = None):
    ...
    if not user:
        return await ctx.send(f"Usage: `{PREFIX}au @user model1,model2`", delete_after=10)
    ...
    if models.lower() == "reset":
        ...
        return await ctx.send(f"✅ Reset {user.mention}\n-# *{WATERMARK}*", delete_after=5)
    ...
    await ctx.send(f"✅ {user.mention}: `{','.join(valid)}`\n-# *{WATERMARK}*", delete_after=5)
...
async def cmd_panel(ctx):
    ...
    if CONFIG_PANEL_URL:
        ...
        await ctx.send("❌ Panel not configured", delete_after=5)
...
def run_flask():
    ...
    def home():
        if bot.user:
            return f"Bot {bot.user} running! | {WATERMARK}"
        return "Starting..."
    
    @app.route('/health')
    def health():
        return jsonify({"status": "ok", "bot_ready": bot.user is not None})
...
if __name__ == "__main__":
    ...
    if HAS_WEB_PANEL and start_web_panel:
        start_web_panel(host="0.0.0.0", port=PORT, admin_key=os.getenv("ADMIN_KEY", "admin123"))
