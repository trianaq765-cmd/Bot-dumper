...
BOT_SECRET=os.getenv("CONFIG_BOT_SECRET")or os.getenv("BOT_SECRET")or"bot_secret_123"
...
def auth_bot(req:Request):
 if not secrets.compare_digest(req.headers.get("X-Bot-Secret",""),BOT_SECRET):
...
def page(title,content,active=""):
 ...
 return f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-900 text-white"><div class="flex min-h-screen"><aside class="w-64 bg-gray-800 p-4 space-y-2 hidden md:block"><h1 class="text-xl font-bold text-blue-400 mb-6 text-center">🤖 Bot Panel</h1>{n}</aside><main class="flex-1 p-6">{content}</main></div></body></html>'''
